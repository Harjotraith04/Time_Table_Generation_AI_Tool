...
function getEstimatedTime(algorithm, parameters = {}) {
  switch (algorithm) {
    ...
    case 'simulated_annealing':
      ...
      const estimatedSA = Math.ceil(maxIter / 1000) * 10;
...
router.post('/recommend', async (req, res) => {
  try {
    ...
    // Default to genetic algorithm if no specific recommendations
    if (recommendations.length === 0) {
      recommendations.push({
        algorithm: 'genetic',
        reason: 'General purpose algorithm suitable for most scenarios',
        confidence: 'medium',
        estimatedTime: '2-5 minutes'
      });
    }
